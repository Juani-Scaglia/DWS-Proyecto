...
func crearVenueAPI(t *testing.T) uint {
	t.Helper()
	token := tokenParaAdmin()
	body := `{"nombre":"Venue Temp","direccion":"Calle Test 1","cap_tribuna_norte":10}`
...
func TestCreateVenueAdmin_Exitoso(t *testing.T) {
	token := tokenParaAdmin()
	body := `{"nombre":"Nuevo Venue","direccion":"Av. Test 100","cap_tribuna_norte":50}`
	req, _ := http.NewRequest("POST", "/api/admin/venues", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+token)
	w := httptest.NewRecorder()
...
func TestUpdateVenueAdmin_IDInvalido(t *testing.T) {
	token := tokenParaAdmin()
	body := `{"nombre":"X","direccion":"X","cap_tribuna_norte":1}`
...
func TestUpdateVenueAdmin_Exitoso(t *testing.T) {
	id := crearVenueAPI(t)
	token := tokenParaAdmin()
	body := `{"nombre":"Venue Actualizado","direccion":"Nueva Dir","cap_tribuna_norte":24}`
